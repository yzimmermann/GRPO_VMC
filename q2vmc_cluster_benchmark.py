...
def make_run_id(architecture: str, system_key: str) -> str:
    return f"{architecture}__{system_key}"


def build_runs(
    architectures: Iterable[str], systems: Iterable[str], preset: BenchmarkPreset
) -> list[RunSpec]:
    runs = []
    for architecture in architectures:
        for system_key in systems:
            ...
            runs.append(
                RunSpec(
                    run_id=make_run_id(architecture, system_key),
...
def build_lapnet_command(run: RunSpec) -> str:
    flags = [
        ...
        f"--config.optim.optimizer={run.preset.optimizer}",
    ...
    for run in runs:
        ...
        metadata = {
            ...
            "wandb": {
                ...
                "tags": [run.preset.name, run.architecture, run.system_key],
            },
        }
...
python -m pip install -e "$LAPJAX_ROOT"
python -m pip install --no-deps -e "$LAPNET_ROOT"
...
def write_readme(outdir: Path, runs: list[RunSpec], args: argparse.Namespace) -> None:
    systems = ", ".join(run.system_key for run in runs[: len(DEFAULT_SYSTEMS)])
    content = textwrap.dedent(
        ...
        - Architectures: {", ".join(sorted(set(run.architecture for run in runs)))}
        - Systems: {systems}
        - Preset: `{args.preset}`
        ...
        4. Summarize finished runs:
           ...
           `python q2vmc_cluster_benchmark.py summarize --outdir {shlex.quote(str(outdir))}`
...
def handle_generate(args: argparse.Namespace) -> int:
    ...
    runs = build_runs(args.architectures, args.systems, preset)
